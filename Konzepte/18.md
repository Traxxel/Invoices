# Aufgabe 18: Retention Policy (optional)

## Ziel

Retention Policy Service für automatische Bereinigung alter Dateien und Datenbank-Einträge.
        ...
        try
        {
            ...
            foreach (var invoice in expiredInvoices)
            {
                ...
                if (await _fileStorageService.FileExistsAsync(invoice.SourceFilePath))
                {
                    ...
                    File.Copy(invoice.SourceFilePath, archiveFilePath);
                }
...
{
    public static IServiceCollection AddRetentionPolicyServices(this IServiceCollection services)
    {
...
- Konfigurierbare Retention Policy (Standard: 10 Jahre)
- Automatische Bereinigung von Dateien und Datenbank-Einträgen
- Archivierung vor Löschung für Backup
- Retention Analysis für Monitoring
- Cleanup Result mit detaillierten Statistiken
- Error Handling für alle Cleanup-Operationen
- Logging für alle Retention-Operationen
- Expired Items Detection
- Space Calculation für Storage-Monitoring
- Archive/Restore-Funktionalität
