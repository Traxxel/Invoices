# Aufgabe 32: Parser-Helpers (Date, Decimal, Address-Block)

## Ziel

Parser-Helpers für die Verarbeitung und Normalisierung von Datumsangaben, Dezimalzahlen und Adressblöcken.
...
public class ParserHelperService : IParserHelperService
{
    ...
    public async Task<AddressValidationResult> ValidateAddressAsync(string addressString)
    {
        try
        {
            ...
            var address = parseResult.Address.Value;
    ...
    public async Task<TextValidationResult> ValidateTextAsync(string text, TextValidationOptions options)
    {
        try
        {
            ...
            {
                var invalidCharacters = text.Where(c => !options.AllowedCharacters.Contains(c)).ToList();
...
- Vollständiger Parser Helper Service für Date, Decimal, Address und Text
- Culture-aware Parsing für verschiedene Sprachen
- Validation für alle Parser-Operationen
- Formatting für alle Parser-Operationen
- Text Normalization und Cleaning
- Text Extraction mit Regex-Patterns
- Error Handling für alle Parser-Operationen
- Logging für alle Parser-Operationen
- Options für konfigurierbare Parser-Parameter
- Warning und Error Management
- Performance-optimierte Parser-Operationen
