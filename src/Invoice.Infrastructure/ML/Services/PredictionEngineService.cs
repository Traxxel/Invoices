    ...
    public async Task<BatchPredictionResult> PredictBatchAsync(List<ExtractedFeature> features)
    {
        try
        {
            ...
            var result = new BatchPredictionResult
            {
                ...
                AverageConfidence = predictions.Any() ? predictions.Average(p => p.Confidence) : 0f,
                MinConfidence = predictions.Any() ? predictions.Min(p => p.Confidence) : 0f,
                MaxConfidence = predictions.Any() ? predictions.Max(p => p.Confidence) : 0f,
                TotalPredictionTime = totalTime,
                AveragePredictionTime = predictions.Any() ? (float)totalTime.TotalMilliseconds / predictions.Count : 0f,
