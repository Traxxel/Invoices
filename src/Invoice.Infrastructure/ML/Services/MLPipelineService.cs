...
public class MLPipelineService : IMLPipelineService
{
    ...
    public IEstimator<ITransformer> CreateFeaturePipeline()
    {
        try
        {
            ...
            IEstimator<ITransformer> pipeline = _mlContext.Transforms.Concatenate("Features", GetFeatureColumns());
    ...
    public async Task<ModelEvaluation> EvaluateModelAsync(ITransformer model, IDataView testData)
    {
        try
        {
            ...
            var evaluation = new ModelEvaluation
            {
                ...
                MicroF1Score = (float)metrics.MicroAccuracy,
                MacroF1Score = (float)metrics.MacroAccuracy,
                WeightedF1Score = (float)metrics.MacroAccuracy,
                PerClassF1Score = new Dictionary<string, float>(),
                PerClassPrecision = new Dictionary<string, float>(),
                PerClassRecall = new Dictionary<string, float>(),
    ...
    public async Task<CrossValidationResult> CrossValidateAsync(IDataView data, int folds = 5)
    {
        try
        {
            ...
            var accuracies = crossValidationResults.Select(r => (float)r.Metrics.MacroAccuracy).ToArray();
            var microF1Scores = crossValidationResults.Select(r => (float)r.Metrics.MicroAccuracy).ToArray();
            var macroF1Scores = crossValidationResults.Select(r => (float)r.Metrics.MacroAccuracy).ToArray();
            ...
            var result = new CrossValidationResult
            {
                ...
                {
                    Accuracy = (float)r.Metrics.MacroAccuracy,
                    MicroF1Score = (float)r.Metrics.MicroAccuracy,
                    MacroF1Score = (float)r.Metrics.MacroAccuracy,
                    WeightedF1Score = (float)r.Metrics.MacroAccuracy
                }).ToList()
            };
    ...
    private Invoice.Application.Interfaces.ConfusionMatrix CreateConfusionMatrix(IDataView predictions)
    {
        // Simplified confusion matrix creation
        // In a real implementation, you would need to extract actual vs predicted labels
        return new Invoice.Application.Interfaces.ConfusionMatrix
        {
